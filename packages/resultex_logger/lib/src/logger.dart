...
class ResultexLogger implements LoggerService {
  /// Creates an [ResultexLogger] instance.
  ResultexLogger();
  ...
  void group(String groupName, void Function() block) {
    developer.log(
      _indent + _colorize('▶️ START: $groupName', _cyan),
      name: 'GROUP',
    );
    ...
    try {
      ...
      developer.log(
        _indent + _colorize('◀️ END: $groupName', _cyan),
        name: 'GROUP',
      );
    }
  }
  ...
  void _logSmartLines(
    ...
    if (lines.length == 1) {
      developer.log(
        _indent + _colorize(message, colorCode),
        name: name,
        level: level,
      );
      return;
    ...
    developer.log(
      _indent + _colorize('└─── End of $name Block ───', colorCode),
      name: name,
      level: level,
    );
  }
